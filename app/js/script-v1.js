...
const tabAllModeListener = function () {
  startApp('all')
  tabAllMode.classList.add('active')
  tabAllMode.removeEventListener('click', tabAllModeListener)
  tabErrorMode.classList.remove('active')


}

const tabErrorModeListener = function () {
  startApp('error')
  tabErrorMode.classList.add('active')
  tabErrorMode.removeEventListener('click', tabErrorModeListener)
  tabAllMode.classList.remove('active')
...
function startApp(appMode) {
  ...
  if (currentAppMode === 'all') {
    console.log('allMode');
    tabErrorMode.addEventListener('click', tabErrorModeListener);
    tabAllMode.classList.add('active')
    ...
    tabAllMode.addEventListener('click', tabAllModeListener);
    tabErrorMode.classList.add('active')

    if (localStorage.getItem('isRandomError') !== null) {
      isRandomError = JSON.parse(localStorage.getItem('isRandomError'));
    } else {
      isRandomError = true
      localStorage.setItem('isRandomError', isRandomError);
    }
    drawCurrentCard(getCurrentCard());
...
function getNextCard(appMode = currentAppMode) {
  if (appMode === 'all') {
    ...
    console.log('nextCard error mode');
    if (localStorage.getItem('answeredCardsAll') !== null) {
      answeredCardsAll = JSON.parse(localStorage.getItem('answeredCardsAll'));
      remainingCards = Object.fromEntries(
        Object.entries(answeredCardsAll).filter(([key, value]) =>
          value[1] === false && value[2] === undefined));
      ...
      if (isRandomAll === true) {
        ...
        // console.log('random')
        nextCard = randomCard
        console.log('Next card for error being randomly generated')
      }
